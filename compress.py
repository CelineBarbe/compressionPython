...
def colle(split_text): 
    glue_text = " ".join(split_text)
    return glue_text
...
testlist = ['mais', 'ceci', 'est', 'un', 'long', 'faux-texte']

def replace_words(split_text : List[str], dict_text : Dict) -> List[str]:
    """on cherche à savoir si le mot est dans le dict
    on modifie ses occurences 
    """
    for key, value in dict_text.items():
        if key in split_text:
           word_index = split_text.index(key)
           split_text[word_index] = value
    return split_text

split_text = replace_words(split_text, dict_text)
print(split_text)


#inversion du dictionnaire
dict_text = {v: k for k, v in dict_text.items()}
print(dict_text)

# décompresser en rechangeant les mots
split_text = replace_words(split_text, dict_text)
glutext = colle(split_text)
print(glutext)

# compter le nombre d'occurence de chaque mot
listtests = ", ".join(['ceci', 'est', 'un', 'faux-texte', 'ceci', 'est'])
def decompte(listtests):
    compte = collections.Counter(listtests.split())
    return compte

decompted = decompte(text)
print(decompted)