...
def replacewords(splittext, dicttext):
    #on cherche à savoir si le mot est dans le dict
    # on modifie ses occurences 
    for k, v in dicttext.items():
        if k in splittext:
           word_index = splittext.index(k)
           splittext[word_index] = v
    return splittext

splittext = replacewords(splittext, dicttext)
print(splittext)


#inversion du dictionnaire
dicttext = {v: k for k, v in dicttext.items()}
print(dicttext)

# décompresser en rechangeant les mots
splittext = replacewords(splittext, dicttext)
gluetext = colle(splittext)
print(gluetext)
